...
@functools.partial(jax.jit, static_argnames=('actor_def', "deterministic"))
def _sample_actions(rng: PRNGKey,
    ...
    if not deterministic:
        ...
        if isinstance(dist, tfd.MultivariateNormalDiag):
            mean = dist.mean()
        elif isinstance(dist, tfd.TransformedDistribution):
            mean = dist.bijector(dist.distribution.mean())
        else:
            raise TypeError(f'Invalid dist type: {type(dist)}')
        return rng, mean
